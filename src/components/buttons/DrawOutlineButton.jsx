...
const Example = (props) => {
  return (
    <DrawOutlineButton>{props.text}</DrawOutlineButton>
  );
};
...
const DrawOutlineButton = ({ children, ...rest }) => {
  return (
    <button
      {...rest}
      className={`group relative px-4 py-2 font-medium text-slate-100 transition-colors duration-[400ms] hover:text-blue-300 hover:cursor-pointer`}
    >
      <span>{children}</span>
